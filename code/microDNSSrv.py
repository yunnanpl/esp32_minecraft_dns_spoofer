...
Not important changes by Dr.JJ for the ESP32 Minecraft DNS spoofer project (2025)
"""
                ...
                if domName :
                    ...
                    if not ipB :
                        for domChk in self._domList.keys() :
                            ### take from override list
                            if domChk.find('*') >= 0 :
                                r = domChk.replace('.', '\.').replace('*', '.*') + '$'
                                if match(r, domName) :
                                    ipB = self._domList.get(domChk, None)
                                    #print( 'spoofing', domName )
                                    break
                        if not ipB :
                            ipB = self._domList.get('*', None)
                        if not ipB :
                            ipB = MicroDNSSrv._ipV4StrToBytes( socket.getaddrinfo( domName, 0 )[0][4][0] )
                            #print( 'forwarding', domName )
                    if ipB :
        ...
        if domainsList and isinstance(domainsList, dict) :
            ...
            for dom, ip in domainsList.items() :
                if isinstance(dom, str) and len(dom) > 0 :
                    ### added
                    #print( ip, ip[-1], ip[-1].isdigit() )
                    try:
                        if ip[-1].isdigit() == False :
                            ### if not in the list, then obtain from normal DNS
                            ip = str( socket.getaddrinfo( ip, 0 )[0][4][0] )
                        ipB = MicroDNSSrv._ipV4StrToBytes(ip)
                        if ipB :
                            o[dom.lower()] = ipB
                            continue
                    except Exception as e:
                        ### do not add to list if ip/domain invalid
                        #print( 'Err2:', e )
                        continue
                break
            #if len(o) == len(domainsList) :
            #    self._domList = o
            #    ### maybe print loaded domains
            #    #print( o )
            #    return True
            self._domList = o
        #return False
